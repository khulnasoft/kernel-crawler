...
from .crawler import DISTROS, crawl_kernels
...
class DistroImageValidation(click.Option):
    def __init__(self, *args, **kwargs):
        ...
        kwargs["help"] = (
            kwargs.get("help", "") + "Option is required when distro is " + ", ".join(self.required_if_distro) + "."
        ).strip()
            ...
            if distro_opt.casefold() == tgt_distro.casefold():
                if current_opt:
                    ...
                    raise click.UsageError(
                        "Missing argument: '" + str(self.name) + "' is required with " + str(distro_opt) + " distro."
                    )
...
@click.option(
    "--distro",
    type=click.Choice(sorted(list(DISTROS.keys())) + ["*"], case_sensitive=True),
)
@click.option("--version", required=False, default="")
@click.option(
    "--arch",
    required=False,
    type=click.Choice(["x86_64", "aarch64"], case_sensitive=True),
    default="x86_64",
)
