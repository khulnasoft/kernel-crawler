...
class DebRepository(repo.Repository):
    ...
    def is_kernel_package(cls, dep):
        return (
            cls.KERNEL_PACKAGE_PATTERN.search(dep)
            and not dep.endswith("-dbg")
            and "modules-extra" not in dep
    ...
    def get_raw_package_db(self):
        ...
        if repo_packages:
            ...
            for _name, details in packages.items():
    ...
    def build_package_tree(cls, packages, package_list):
        ...
        with click.progressbar(
            package_list,
            label="Building dependency tree",
            file=sys.stderr,
            item_show_func=repo.to_s,
        ) as pkgs:
            for pkg in pkgs:
...
class DebMirror(repo.Mirror):
    ...
    def list_repos(self):
        ...
        dists = [
            dist
            for dist in doc.xpath('/html/body//a[not(@href="../")]/@href')
            if dist.endswith("/")
        ...
        with click.progressbar(
            dists,
            label="Scanning {}".format(self.base_url),
            file=sys.stderr,
            item_show_func=repo.to_s,
        ) as dists:
            for dist in dists:
