...
class TalosMirror(GitMirror):
    ...
    def get_package_tree(self, version=""):
        ...
        self.pkgs_repo = pygit2.clone_repository(
            "https://github.com/siderolabs/pkgs.git",
            work_dir,
            callbacks=ProgressCallback("pkgs"),
        )
        ...
        for v in talos_versions:
            ...
            bar = ProgressBar(
                label="Building config for talos v{}".format(v),
                length=1,
                file=sys.stderr,
            )
            ...
            # makes easier to get the right khulnasoft drivers from within a talos instance.
